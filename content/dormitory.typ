...
#let 主体表格 = table(
  ...
  tcell(rowspan: 5, good[独卫\ +\ 公卫#fn(2)]),
...
#let 表格注 = stack(
  ...
  ..(
    ...
    [雁北 D2, E 楼为一个区域，内部部分连通，共用电梯。D2 楼借用 E 楼公卫和层浴浴室，可能需要前往其他楼层。],
    [雁北 A, B, C, D1 楼为一个区域，内部部分连通。该区域两部电梯均位于 D1 部分，A, B, C 楼需要借道；A, B 二楼与 A 六楼无法通过电梯到达，需要步行一层。],
  )
    .enumerate()
    .map(((n, it)) => fn(n + 1) + it),
)
